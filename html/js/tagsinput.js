...
! function(a) {
    ...
    var h = {
        tagClass: function(a) {
            var items = Array("primary", "info", "success", "warning", "danger");
            return "tag is-" + items[Math.floor(Math.random()*items.length)];
        },
        focusClass: "focus",
                ...
                if (!b.toString().match(/^\s*$/)) {
                    ...
                    if (!n || f.options.allowDuplicates) {
                        if (!(f.items().toString().length + b.length + 1 > f.options.maxInputLength)) {
                            ...
                            var perms = Array("reachDashboard", "reachPosts", "reachUsers", "addUser", "editUsers", "removeUsers", "managePosts", "addPost", "manageCategories", "all");
                            if(!isInArray(perms, k)) {
                                o.cancel = 1;
                                $('.permerror').show();
                                setTimeout(function(){
                                    $('.permerror').hide();
